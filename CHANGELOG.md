...
### Added
- **Search Type Tracking**: Distinguish between full searches and autocomplete suggestions
  - New `search_type` column in `wl_search_log` table (via migration)
  - `SearchLogSubscriber` now captures both `ProductSearchResultEvent` and `ProductSuggestResultEvent`
  - `SearchLogService` provides `type_breakdown` analytics:
    - Today's stats by type (count, unique terms, zero result rate)
    - This week's comparison by type
  - Backwards compatible: returns availability message if migration hasn't run
